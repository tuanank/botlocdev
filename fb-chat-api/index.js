...
try {
    if (!global.Fca.Require.fs.existsSync(process.cwd() + '/FcaConfig.json')) {
        global.Fca.Require.fs.writeFileSync(process.cwd() + "/FcaConfig.json", JSON.stringify(global.Fca.Data.ObjPriyansh, null, "\t"));
        process.exit(1);
    }

try {
    var DataLanguageSetting = require(process.cwd() + "/FcaConfig.json");
}
catch (e) {
    ...
    if (global.Fca.Require.fs.existsSync(process.cwd() + '/FcaConfig.json')) {
        try { 
            if (DataLanguageSetting.Logo != undefined) {
                ...
                global.Fca.Require.fs.writeFileSync(process.cwd() + "/FcaConfig.json", JSON.stringify(DataLanguageSetting, null, "\t"));        
            }
        }
        catch (e) {
            console.log(e);
        ...
        rl.question(Language.TypeAccount, (Account) => {
            if (!Account.includes("@") && global.Fca.Require.utils.getType(parseInt(Account)) != "Number") return logger.Normal(Language.TypeAccountError, function () { process.exit(1) }); //Very Human
                    ...
                    if (global.Fca.Require.Priyansh.ResetDataLogin) {
                        ...
                        global.Fca.Require.fs.writeFileSync(process.cwd() + '/FcaConfig.json', JSON.stringify(global.Fca.Require.Priyansh, null, 4));
                    }
