...
function getAppState(jar, Encode) {
    ...
    var data;
        switch (require(process.cwd() + "/FcaConfig.json").EncryptFeature) {
            case true: {
            ...
            case false: {
                ...
                break;
            default: {
                logger.Normal(getText(Language.IsNotABoolean,require(process.cwd() + "/FcaConfig.json").EncryptFeature));
                data = appstate;
            } 
        }
            if(!globalThis.Fca.Setting.get('getAppState')) {
