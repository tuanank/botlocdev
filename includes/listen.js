   ...
   try {
    ...
    if (thu == 'Sunday') thu = 'Chủ nhật'
     ...
     if (((global.data.threadData.get(event.threadID)?.PREFIX || global.config.PREFIX) + 'bank') != event.args[0]) {
        if (!find_thuebot) return send(`❎ Nhóm của bạn chưa thuê bot, vui lòng liên hệ Admin Thanh Lộc để thuê bot\n🌐 Facebook: ${config.FACEBOOK_ADMIN}\n🔐 Zalo: ${config.ZALO_ADMIN}`);
        if (new Date(form_mm_dd_yyyy(find_thuebot.time_end)).getTime() <= Date.now() + 25200000) return send(`⚠️ Nhóm của bạn đã hết hạn thuê bot, vui lòng thanh toán để tiếp tục gia hạn, liên hệ Admin Phạm Hữu Tài\n🌐 Facebook: ${config.FB_ADMIN}\n🔐 Zalo: ${config.ZALO_ADMIN}`);
     };
  };
