...
module.exports.handleReply = async function ({ event, api, handleReply }) {
    ...
    switch (handleReply.type) {
      ...
      case "create": {
       ...
       return request(encodeURI(`https://docs-api.jrtxtracy.repl.co/taoanhdep/avatarwibu?id=${id}&chu_nen=${nen}&chu_ky=${ky}`)).pipe(fs.createWriteStream(__dirname+'/cache/tad.png')).on('close',() => callback());    
    }
   }
 }
...
if (args[0] == "list") {
	...
	const res = await axios.get(`https://docs-api.jrtxtracy.repl.co/taoanhdep/search?key=${nhanvat}`);
...
return api.sendMessage(`[⚜️] ➜ Nhân Vật: ${text} \n[⚜️] ➜ ID: ${idz}\n[⚜️] ➜ Màu Mặc Định: ${color}\n[⚜️] ➜ Nguồn: ${source}`, event.threadID, event.messageID)
       ...
       const ress = await axios.get('https://docs-api.jrtxtracy.repl.co/taoanhdep/find')
      var nhanvat = args[1]
