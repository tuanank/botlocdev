...
module.exports.run = async function ({ api, args, event, permssion }) {
  ...
  const { threadID, messageID, senderID, body } = event;
	 if (this.config.credits != 'JRT') {
        console.log('\x1b[33m[ WARN ]\x1b[37m [⚜️] ➜ Bớt đổi credits đê mau sửa lại !!!');
    ...
    return api.sendMessage(`[⚜️] ➜ Reply tin nhắn này để nhập họ và tên của bạn`, event.threadID, (err, info) => {
    ...
    switch (handleReply.type) {
        ...
        case "create": {
            ...
            setTimeout(() => {
                ...
                return request(encodeURI(`${global.configApi.domain1}/m2022?name=${hi}&uid=${senderID}`)).pipe(fs.createWriteStream(__dirname + '/cache/fbcover.png')).on('close', () => callback());
            }, 1000);
          }, messageID);
        }
    }} 