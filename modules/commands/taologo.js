...
var lengthchar = JSON.parse(bytes.toString(CryptoJS.enc.Utf8));
    ...
    switch(handleReply.step) {
        case 1: {
            ...
            return api.sendMessage(`[⚜️]➜ Bạn chọn logo mang ID ${event.body}, hãy reply tin nhắn này để nhập tên`, event.threadID, (err, info) => {
        ...
        case 2: {
            api.unsendMessage(handleReply.messageID);
            return api.sendMessage(`[⚜️]➜ Bạn chọn tên  ${event.body}, hãy reply tin nhắn này để nhập Slogan`, event.threadID, (err, info) => {
                global.client.handleReply.push({
                    step: 3,
                    name: "taologo",
                    author: event.senderID,
		...
		case 3: {
            api.unsendMessage(handleReply.messageID);
            return api.sendMessage(`[⚜️]➜ Bạn đã nhập Slogan là ${event.body}, hãy reply tin nhắn này để nhập color1`, event.threadID, (err, info) => {
                global.client.handleReply.push({
                    step: 4,
                    name: "taologo",
                    author: event.senderID,
		...
		case 4: {
            api.unsendMessage(handleReply.messageID);
            return api.sendMessage(`[⚜️]➜ Bạn đã nhập color1 là ${event.body}, hãy reply tin nhắn này để nhập color2`, event.threadID, (err, info) => {
                global.client.handleReply.push({
                    step: 5,
                    name: "taologo",
                    author: event.senderID,
...
const { loadImage, createCanvas } = require("canvas");
  ...
  if (!fs.existsSync(__dirname +
    ...
    return api.sendMessage("[⚜️]➜ Reply Tin Nhắn Này Để Chọn logo", event.threadID, (err, info) => {
        global.client.handleReply.push({
            step: 1,
            name: "taologo",
            author: event.senderID,
